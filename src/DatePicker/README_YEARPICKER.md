...
| `value`    | `number[] \| { startDate: number \| null; endDate: number \| null }`                  | `-`        | Pilihan yang ditandai saat sheet dibuka; halaman ikut berpindah ke tahun tersebut. Bentuknya sama persis dengan yang dilaporkan `onChange`. Jika tidak diisi, picker hanya mengingat pilihan dari tap-nya sendiri. |
