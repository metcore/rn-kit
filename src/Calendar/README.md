...
| `disabledDates`           | `(string \| Date)[]`                   | `['2025-06-10', new Date(2025, 5, 11)]`               | Tanggal tertentu yang tidak bisa dipilih. Pakai ini, bukan `markedDates` dengan `disabled: true` — mark juga mengatur warna, jadi menonaktifkan lewat mark akan berebut dengan warna yang sudah dipasang pemanggil (atau oleh DatePicker untuk tanggal terpilih). |
...
| `todayTextColor`          | `string` (CSS color)                   | `Color.info[500]`                                     | Warna angka tanggal hari ini. Hanya berlaku bila hari ini tidak sedang dipilih, dinonaktifkan, atau ditandai lewat `markedDates` — ketiganya menang atas ini. |
